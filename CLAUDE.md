...
| Tool parameter changed | `docs/wiki/Available-Tools.md` (update parameter table — the lockstep test `test/unit/docs/available-tools-params-sync.test.ts` fails until it lists exactly the schema's parameters), `CHANGELOG.md` (`[Unreleased]` → Changed) |
| `routers.yaml` / `identities.yaml` schema changed | `docs/wiki/Configuration.md` and `config/*.example.yaml` (the lockstep test `test/unit/docs/configuration-yaml-sync.test.ts` loads every full YAML example on that page through the real registries) |
