...
describe("interfaceListTools", () => {
  describe("metadata", () => {
    ...
    it("list_interface_lists is readOnly", () =>
      expect(listListsTool.annotations.readOnlyHint).toBe(true));
  ...
  describe("list_interface_lists", () => {
    ...
    it("rejects extra fields", () => {
      expect(listListsTool.inputSchema.safeParse({ routerId: "r1", extra: true }).success).toBe(
        false,
      );
    });
  });
  ...
  describe("manage_interface_list", () => {
    it("creates list when not found", async () => {
      ...
      const result = await manageListTool.handler(
        { routerId: "test-router", action: "add", name: "WAN" },
        ctx,
      );
    ...
    it("removes list when found", async () => {
      const ctx = makeContext([LIST1]);
      const result = await manageListTool.handler(
        { routerId: "test-router", action: "remove", name: "WAN" },
        ctx,
      );
    ...
    it("dry_run returns preview without create", async () => {
      const ctx = makeContext([]);
      const result = await manageListTool.handler(
        { routerId: "test-router", action: "add", name: "WAN", dryRun: true },
        ctx,
      );
    ...
    it("rejects invalid action", () => {
      expect(
        manageListTool.inputSchema.safeParse({ routerId: "r1", action: "enable", name: "WAN" })
          .success,
      ).toBe(false);
    });
    it("rejects extra fields", () => {
      expect(
        manageListTool.inputSchema.safeParse({
          routerId: "r1",
          action: "add",
          name: "WAN",
          extra: true,
    ...
    it("dry_run on remove returns preview without deleting", async () => {
      const ctx = makeContext([LIST1]);
      const result = await manageListTool.handler(
        { routerId: "test-router", action: "remove", name: "WAN", dryRun: true },
        ctx,
      );
      expect((result.structuredContent as Record<string, unknown>).action).toBe("dry_run");
  ...
  describe("manage_interface_list_member", () => {
    it("adds member when not found", async () => {
      ...
      const result = await manageMemberTool.handler(
        { routerId: "test-router", action: "add", list: "WAN", interface: "ether1" },
        ctx,
      );
      expect((result.structuredContent as Record<string, unknown>).action).toBe("created");
      expect(ctx.routerClient.create).toHaveBeenCalledWith(
        "interface/list/member",
        expect.objectContaining({ list: "WAN", interface: "ether1" }),
      );
    });
    ...
    it("removes member when found", async () => {
      const ctx = makeContext([], [MEMBER1]);
      const result = await manageMemberTool.handler(
        { routerId: "test-router", action: "remove", list: "WAN", interface: "ether1" },
        ctx,
      );
      expect((result.structuredContent as Record<string, unknown>).action).toBe("removed");
    ...
    it("dry_run returns preview without create", async () => {
      const ctx = makeContext([], []);
      const result = await manageMemberTool.handler(
        { routerId: "test-router", action: "add", list: "WAN", interface: "ether1", dryRun: true },
        ctx,
      );
      expect((result.structuredContent as Record<string, unknown>).action).toBe("dry_run");
      expect(ctx.routerClient.create).not.toHaveBeenCalled();
    ...
    it("rejects extra fields", () => {
      expect(
        manageMemberTool.inputSchema.safeParse({
          routerId: "r1",
          action: "add",
          list: "WAN",
          interface: "ether1",
