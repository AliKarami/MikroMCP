...
describe("networkTestTools", () => {
  ...
  describe("bandwidth_test input schema", () => {
    it("requires address", () =>
      expect(bandwidthTestTool.inputSchema.safeParse({ routerId: "r1" }).success).toBe(false));
    it("defaults protocol tcp", () =>
      expect(
        bandwidthTestTool.inputSchema.parse({ routerId: "r1", address: "10.0.0.1" }).protocol,
      ).toBe("tcp"));
    it("defaults direction both", () =>
      expect(
        bandwidthTestTool.inputSchema.parse({ routerId: "r1", address: "10.0.0.1" }).direction,
      ).toBe("both"));
    it("defaults duration 5", () =>
      expect(
        bandwidthTestTool.inputSchema.parse({ routerId: "r1", address: "10.0.0.1" }).duration,
      ).toBe(5));
    it("rejects duration above 30", () =>
      expect(
        bandwidthTestTool.inputSchema.safeParse({
          routerId: "r1",
          address: "10.0.0.1",
          duration: 60,
        }).success,
  ...
  describe("fetch_url input schema", () => {
    it("requires url", () =>
      expect(fetchUrlTool.inputSchema.safeParse({ routerId: "r1" }).success).toBe(false));
    it("defaults method GET", () =>
      expect(
        fetchUrlTool.inputSchema.parse({ routerId: "r1", url: "http://example.com" }).method,
      ).toBe("GET"));
    it("rejects extra fields", () =>
      expect(
        fetchUrlTool.inputSchema.safeParse({ routerId: "r1", url: "http://x.com", extra: true })
          .success,
      ).toBe(false));
  });
  ...
  describe("fetch_url handler", () => {
    ...
    it("returns outputFile path instead of body when outputFile provided", async () => {
      ...
      const result = await fetchUrlTool.handler(
        { routerId: "test-router", url: "http://example.com", outputFile: "flash/response.txt" },
        ctx,
      );
      const sc = result.structuredContent as Record<string, unknown>;
    ...
    it("truncates body over 64KB", async () => {
      ...
      const ctx = makeContext([
        { ".section": "0", status: "finished", code: "200", data: bigBody },
      ]);
      const result = await fetchUrlTool.handler(
        { routerId: "test-router", url: "http://example.com" },
        ctx,
  ...
  describe("list_connections", () => {
    it("returns all connections without filter", async () => {
      ...
      expect(
        ((result.structuredContent as Record<string, unknown>).connections as unknown[]).length,
      ).toBe(2);
    });
    ...
    it("filters by protocol", async () => {
      ...
      const result = await listConnectionsTool.handler(
        { routerId: "test-router", protocol: "tcp" },
        ctx,
      );
      expect(
        ((result.structuredContent as Record<string, unknown>).connections as unknown[]).length,
    ...
    it("filters by srcAddress substring", async () => {
      const ctx = makeContext({}, [
        ...
        {
          ".id": "*2",
          "src-address": "192.168.1.5:8080",
          "dst-address": "1.1.1.1:80",
          protocol: "tcp",
        },
      ]);
      const result = await listConnectionsTool.handler(
        { routerId: "test-router", srcAddress: "192.168" },
        ctx,
      );
      expect(
        ((result.structuredContent as Record<string, unknown>).connections as unknown[]).length,
