...
describe("vlanTools", () => {
  ...
  describe("manage_vlan - add", () => {
    ...
    it("returns already_exists when the record carries a parsed numeric vlan-id", async () => {
      // The response parser converts the vlan-id wire string to a number — the
  ...
  describe("manage_vlan - remove", () => {
    ...
    it("removes a purely numeric name that the parser turned into a number", async () => {
      // A VLAN legally named "100" comes back from the response parser as the
      // number 100 — the identity match must still find it.
      const ctx = makeContext([{ ".id": "*1", name: 100, "vlan-id": "10", interface: "ether1" }]);
      const result = await manageTool.handler(
        { routerId: "test-router", action: "remove", name: "100" },
        ctx,
      );
      const sc = result.structuredContent as Record<string, unknown>;
      expect(sc.action).toBe("removed");
