...
- **Docs:** the Docker quick start never set `MIKROMCP_CONFIG_PATH` / `MIKROMCP_IDENTITIES_PATH`, so the container ignored the mounted `config/` directory and started with an empty registry; the Claude Code HTTP registration lacked the required `Authorization` header. Also corrected: when `MIKROMCP_CONFIRMATION_SECRET` is required and which roles the confirmation gate applies to (admins bypass only the per-router gate; destructive `bulk_execute` fan-outs need the secret and a fleet token from every role), that `bulk_execute` refuses `check_router_health` as an inner tool, `/metrics` authentication, `upload_file` policies (SFTP needs `ssh`), the usage-skill install steps, the Development page's source tree (now lists `middleware/`, `util/`, the SwOS adapters, and every tool file), the `run_command` per-router override example, and the wiki Roadmap's stale "v1.9 is the current release" line. The Configuration page gained a router-field reference and a maintenance-window example.
- `rollback_change`'s not-found hint referenced a `MIKROMCP_JOURNAL_PATH` variable that does not exist; it now points at the write journal under `MIKROMCP_DATA_DIR`.
