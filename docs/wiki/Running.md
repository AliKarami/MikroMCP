...
| `/metrics` | GET | Only when identities are configured (Bearer token) | Prometheus metrics exposition — returns per-tool call counters (`mikromcp_tool_calls_total`). Not rate-limited. Suitable for Prometheus scraping. |
